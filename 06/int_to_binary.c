...
void instruction_to_bits(char* str, char* dest, char* comp, char* jump, FILE *output_file) {
	...
	if (c_bits == 0xFFFF || d_bits == 0xFFFF || j_bits == 0xFFFF) {
		fprintf(stderr, "Invalid C-instruction fields: dest='%s' comp='%s' jump='%s'\n", dest, comp, jump);
		exit(1);
	}
	if (strchr(comp, 'M'))
		instruction |= (1 << 12);
	...
	int_to_binary(binary_buffer, instruction, output_file);
